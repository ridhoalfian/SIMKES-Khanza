...
public final class RMDataResumePasien extends javax.swing.JDialog {
        ...
        if(tabMode.getRowCount()==0){
            ...
            Map<String, Object> param = new HashMap<>(); 
                ...
                if(TCari.getText().equals("")){
                    Valid.MyReport("rptDataResumePasien.jrxml","report","::[ Data Resume Pasien ]::",
    ...
    private void MnLaporanResumeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnLaporanResumeActionPerformed
        if(tbObat.getSelectedRow()>-1){
            ...
            Valid.MyReport("rptLaporanResume.jrxml","report","::[ Laporan Resume Pasien ]::","select reg_periksa.tgl_registrasi,reg_periksa.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.jk,pasien.tgl_lahir,\n" +
...
"where resume_pasien.no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),2).toString()+"' ");
        }
        ...
        if(tabMode.getRowCount()==0){
            ...
            if(tbObat.getSelectedRow()>-1){
                if(!tbObat.getValueAt(tbObat.getSelectedRow(),1).toString().equals("")){
                    ...
                    try {
                        //berkas.loadURL("http://"+koneksiDB.HOST()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/"+"berkasrawat/login2.php?act=login&usere=admin&passwordte=akusayangsamakamu&no_rawat="+tbObat.getValueAt(tbObat.getSelectedRow(),2).toString());
                        berkas.loadURL("http://"+koneksiDB.HOST()+":"+prop.getProperty("PORTWEB")+"/"+prop.getProperty("HYBRIDWEB")+"/"+"berkasrawat/login2.php?act=login&usere=admin&passwordte=akusayangsamakamu&no_rawat="+tbObat.getValueAt(tbObat.getSelectedRow(),2).toString());
                    } catch (Exception ex) {
    ...
    */
    ...
    public void isCek(){
        ...
        if(var.getjml2()>=1){
            ...
            KodeDokter.setText(var.getkode());

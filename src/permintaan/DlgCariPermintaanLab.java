...
public class DlgCariPermintaanLab extends javax.swing.JDialog {
    ...
    private String tglsampel="",tglhasil="",norm="",kamar="",namakamar="",diagnosa="",pengirim="",la="",ld="",pa="",pd="";
    ...
    private void MnCetakHasilLabActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakHasilLabActionPerformed
        if(tbDokter.getSelectedRow()!= -1){
            ...
            if(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString().trim().equals("")){
                ...
                param.put("carabayar",Sequel.cariIsi("select png_jawab from penjab inner join reg_periksa on penjab.kd_pj=reg_periksa.kd_pj where reg_periksa.no_rawat=? ",tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString()));
                ...
                if(!kamar.equals("")){
                    pengirim=Sequel.cariIsi("select nm_dokter from dokter inner join dpjp_ranap on dokter.kd_dokter=dpjp_ranap.kd_dokter inner join reg_periksa on dpjp_ranap.no_rawat = reg_periksa.no_rawat "+
